...
\frame{\frametitle{Asymmmetric Encryption}
	...
	Examples of asymmetric systems are ElGamal, RSA, DSS and Diffie-Hellman key exchange protocol \newline
	\center
...
\frame{\frametitle{Euler's Totient}
	...
	The $\phi$ value of a number is the count of numbers less than it that aren't factors of the original number \newline
...
\frame{\frametitle{Diffie-Hellman Key Exchange}
	...
	\draw[->,draw=black] (aliceMixed) to (bobRecv);
	\draw[->,draw=black] (aliceRecv) to (alicePrivate2);
	...
	\draw[->,draw=black] (bobMixed) to (aliceRecv);
	\draw[->,draw=black] (bobRecv) to (bobPrivate2);
...
\frame{\frametitle{ElGamal Cryptosystem}
	...
	The first step in creating a prime modulous and a generator\newline
	Next create a Public and Private key \newline
